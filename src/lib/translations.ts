...
const translations = {
    ...
    nicknameHelp: { KR: '레이팅 정보를 가져오려면 Chunirec 닉네임이 필요합니다.', JP: 'レーティング情報を取得するにはChunirecのニックネームが必要です。' },
    ...
    // Form Toasts
    toastErrorNicknameNeeded: { KR: '닉네임을 입력해주세요.', JP: 'ニックネームを入力してください。' },
    toastErrorNicknameNeededDesc: { KR: 'Chunirec 닉네임을 입력해야 레이팅 정보를 가져올 수 있습니다.', JP: 'Chunirecのニックネームを入力して、レーティング情報を取得してください。' },
    ...
    toastErrorUserNotFoundDesc: { KR: '{0} 닉네임의 사용자가 Chunirec에 존재하지 않습니다.', JP: '{0}というニックネームのユーザーがChunirecに存在しません。' },
    ...
    toastErrorMissingInfoDesc: { KR: '닉네임과 현재 레이팅을 모두 입력해야 합니다.', JP: 'ニックネームと現在のレーティングを両方入力する必要があります。' },
    ...
    // Tooltips
    tooltipChunirecNicknameContent: { KR: 'Chunirec에 등록된 닉네임을 정확히 입력해주세요.', JP: 'Chunirecに登録されたニックネームを正確に入力してください。' },
    ...
    cacheUserNicknameLabel: { KR: '캐싱할 유저 닉네임', JP: 'キャッシュするユーザー名' },
    cacheUserNicknamePlaceholder: { KR: 'Chunirec 닉네임', JP: 'Chunirec ニックネーム' },
    cacheUserRecordsButton: { KR: '사용자 기록 캐싱', JP: 'ユーザー記録をキャッシュ' },
    deleteSpecificUserDataLabel: { KR: '특정 사용자 데이터 삭제', JP: '特定ユーザーデータ削除' },
    deleteSpecificUserDataDesc: { KR: '입력한 닉네임의 캐시된 기록만 삭제합니다.', JP: '入力されたニックネームのキャッシュ記録のみ削除します。' },
    deleteUserNicknameLabel: { KR: '삭제할 유저 닉네임', JP: '削除するユーザー名' },
    deleteUserNicknamePlaceholder: { KR: 'Chunirec 닉네임', JP: 'Chunirec ニックネーム' },
    ...
    toastErrorNicknameToDeleteNeeded: { KR: '삭제할 사용자의 닉네임을 입력해주세요.', JP: '削除するユーザーのニックネームを入力してください。' },
    toastErrorNicknameToDeleteNeededDesc: { KR: '정확한 닉네임을 입력해야 해당 사용자의 데이터를 삭제할 수 있습니다.', JP: '正確なニックネームを入力すると、そのユーザーのデータを削除できます。' },
